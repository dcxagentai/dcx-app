...
export function DcxAppSendMessagePage(props: Props) {
  ...
  const [sendStage, setSendStage] = useState<"idle" | "preparing" | "uploading" | "processing" | "success" | "error">("idle")
    ...
    <section className="flex min-h-[calc(100vh-5rem)] flex-col gap-4 text-slate-950">
      <section className="border border-black/6 bg-white p-5 shadow-[0_18px_55px_-48px_rgba(15,23,42,0.45)]">
        ...
        <DcxSendProgressPanel
          ...
          isVisible={isSendingMessage || createMessageMutation.isSuccess || createMessageMutation.isError}
          isSuccess={createMessageMutation.isSuccess}
          isError={createMessageMutation.isError}
          errorText={createMessageMutation.isError
...
function DcxSendSelectedAttachmentCard(props: {
  ...
  locked: boolean
  sendingStage: "idle" | "preparing" | "uploading" | "processing" | "success" | "error"
...
function readDcxSendCommentary(
  sendStage: "idle" | "preparing" | "uploading" | "processing" | "success" | "error",
  fileCount: number,
  uxStrings: Record<string, string>,
